...
def _generate_toml(
    ...
    DB; on a match, name/sport/color are overridden and strava_id is recorded.

    Note: the `garmin-{activity_id}` id format is parsed by the frontend
    (src/wishmap/static/index.html popupHtml) to build the Garmin Connect link.
    ...
    for activity in activities:
        ...
        if strava_id is not None:
            lines.append(f"strava_id = {strava_id}")
        lines.append("")
